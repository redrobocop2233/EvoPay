...
# 27. Latest Engineering State — 2026-08-31

The repository was re-verified and the planned evaluation/adaptation features were added.

## Completed in the latest pass

- Held-out primitive-combination evaluation
- Holdout filtering during evolutionary generations
- Separate held-out metrics in `closed_loop_summary.json`
- Adaptive mutation pressure when Blue detection saturates
- Random Forest `max_depth` reduced from 8 to 5
- Seed sweep runner in `experiments/seed_sweep.py`
- Seed 13 selected from a quick three-seed sweep (1, 7, 13)
- Frozen verification run in `experiments/frozen_seed_13/`
- Explicit parent campaign lineage fields
- Human-readable mutation summaries
- `eval/lineage.py`
- `eval/time_to_adapt.py`
- Time-to-Adapt metrics saved to run artifacts
- UI lineage table and Time-to-Adapt / held-out metric cards
- Dashboard lineage tab and Time-to-Adapt metric
- Initial automated tests

## Verification status

The current repository has passed:
...
A deterministic local closed-loop run also completes successfully.

## Frozen run

The current frozen verification configuration is:
...
```json
{
  ...
  "days": 60,
  "use_genai": false,
  "detector_type": "TrainableDetector"
}
```

Frozen run headline results:
...
```text
20 reserved campaigns
65.0% detection
0.0% FPR against untouched legitimate baseline
```

Time-to-Adapt is computed per attack family, but should not be marketed as a production
response-time KPI. It measures the number of evolutionary generations until a family
reaches the configured detection threshold within the synthetic experiment.

## Seed sweep result

Quick-profile sweep across seeds 1, 7 and 13 ranked:

```text
seed 13 → composite 0.7472
seed 1  → composite 0.6899
seed 7  → composite 0.4772
```

Seed 13 is the frozen demo candidate. The seed score is a presentation/experiment
selection heuristic and is not a model-performance metric.
